...
$this->set('site_url', $this->getSiteUrl()); // for pathToUrl(), which is used in thumbnail function

//t($this->getSiteUrl(true));
...
$this->set('monoplane.assets.top', [
    MP_BASE_URL.'/modules/Monoplane/assets/css/style.css', // main style file
    MP_BASE_URL.'/modules/Monoplane/assets/css/responsive.css',
]);
$this->set('mediapath', '/modules/Monoplane/assets/media');
...
$this->bind('/:chips/:slug', function($params) {
    return $this->invoke('Monoplane\\Controller\\Chips', 'index', ['slug' => $params['slug']]);
});
...
$this->renderer->extend(function($content){ // returns relative url of scaled image (image)
    return preg_replace('/(\s*)@imagebk\((.+?)\)/', '$1<?php echo MP_BASE_URL."/getImage?src=".urlencode($2)."&w=".$app->retrieve("monoplane/lexy/image/width", 800)."&h=".$app->retrieve("monoplane/lexy/image/height", 800)."&q=".$app->retrieve("monoplane/lexy/image/quality", 80)."&m=".$app->retrieve("monoplane/lexy/image/mode", "bestFit"); ?>', $content);
});

$this->renderer->extend(function($content){ // returns relative url of scaled image (image)
    return preg_replace('/(\s*)@image\((.+?)\)/', '$1<?php echo MP_BASE_URL."/getImage?src=".$app->retrieve("mediapath").urlencode($2)."&w=".$app->retrieve("monoplane/lexy/image/width", 800)."&h=".$app->retrieve("monoplane/lexy/image/height", 800)."&q=".$app->retrieve("monoplane/lexy/image/quality", 80)."&m=".$app->retrieve("monoplane/lexy/image/mode", "bestFit"); ?>', $content);
});
